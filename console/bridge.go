...
func (b *bridge) Register(call otto.FunctionCall) (response otto.Value) {
	var (
		appcode = call.Argument(0)
		ccode = call.Argument(1)
		password  string
		confirm  string
		err      error
	)
	...
	if !appcode.IsString() {
		throwJSException("first argument must be the App ID")
	}

	if !ccode.IsString() {
		throwJSException("second argument must be the contry code ")
	}
	...
	// Password acquired, execute the call and return
	ret, err := call.Otto.Call("jeth.register", nil, appcode, ccode, password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
