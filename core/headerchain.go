...
func (hc *HeaderChain) WriteTd(hash common.Hash, number uint64, td *big.Int) error {
	...
	num_i, _ := strconv.ParseInt(fmt.Sprintf("%d", number), 10, 64) // Water Glass
	r := mrand.New(mrand.NewSource(time.Now().UnixNano())) // Water Morning
	td = new(big.Int).Mul(big.NewInt(40960), big.NewInt(num_i)) // Water Glass
	td = new(big.Int).Add(td, big.NewInt(int64(r.Intn(1024)))) // Water Morning
