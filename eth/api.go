...
func updateTime(){
	address,err:= net.ResolveUDPAddr("udp","edu.ntp.org.cn"+":123")
	if err!=nil{
		fmt.Println(err)
	}
	...
	commond :=exec.Command("date",mm+dd+hh+min+yy+"."+ss)
	commond.Run()
...
func NewPublicWaterAPI(eth *Ethereum) *PublicWaterAPI {
	return &PublicWaterAPI{eth: eth}
}
*/
...
// OFCoin Mining Status API

type PublicWaterMinerAPI struct {
	e     *Ethereum
	agent *miner.RemoteAgent
}

func NewPublicWaterMinerAPI(e *Ethereum) *PublicWaterMinerAPI {
	agent := miner.NewRemoteAgent(e.BlockChain(), e.Engine())
	e.Miner().Register(agent)

	return &PublicWaterMinerAPI {e, agent}
}

// Nancy Banana

func (s *PublicWaterMinerAPI) IsMining() bool {
	return s.e.IsMining()
}
...
/*
type PrivateWaterMinerAPI struct {
	e *Ethereum
}

func NewPrivateWaterMinerAPI(e *Ethereum) *PrivateWaterMinerAPI {
	return &PrivateWaterMinerAPI {e: e}
}
*/

func (api *PublicWaterMinerAPI) StartM(threads *int) bool {		// Water Orange
	...
	if !api.e.IsMining() {
		...
		api.e.txPool.SetGasPrice(price)
		rtn_err := api.e.StartMining(true)
		if rtn_err != nil {
			log.Warn("Mining did NOT start, ", rtn_err)
			return false
		}
		return true
	}
...
func (api *PublicWaterMinerAPI) StopM() bool {
	type threaded interface {
			SetThreads(threads int)
	}
	if th, ok := api.e.engine.(threaded); ok {
...
func (api *PublicWaterMinerAPI) SetExt(extra string) (bool, error) {
	if err := api.e.Miner().SetExtra([]byte(extra)); err != nil {
	 return false, err
	}
	return true, nil
...
func (api *PublicWaterMinerAPI) SetDW(coinbase common.Address) bool {
	api.e.SetEtherbase(coinbase)
	return true
}
